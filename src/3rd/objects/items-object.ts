...
export default class ItemsObject {
  ...
  public createTimerEvent(): void {
    ...
    // レベル選択 (セレクトボックス)
    const level: Level = 'easy';  // `value` で良いだろ
    ...
    this.itemsTimerEvent = this.scene.time.addEvent({
      ...
      callback: () => {
        ...
        if(isAddItem) {
          ...
          this.items.add(new ItemObject(this.scene, gameWidth + 25, Phaser.Math.Between(25, gameHeight - 100 - 25), Phaser.Math.Between(-600, -300), keyName));
        }
        
        // レアアイテム
        const isAddRareItem = Phaser.Math.Between(0, 50) === 0;
        if(isAddRareItem) {
          this.items.add(new ItemObject(this.scene, gameWidth + 25, Phaser.Math.Between(25, gameHeight - 100 - 25), Phaser.Math.Between(-1100, -800), ItemObject.keyNameUnknown));
        }
        ...
        if(isAddEnemy) this.items.add(new ItemObject(this.scene, -25, Phaser.Math.Between(25, gameHeight - 100 - 25), Phaser.Math.Between(800, 500), ItemObject.keyNameEnemy));
        
        // 爆弾
        const isAddBomb = Phaser.Math.Between(0, 20) === 0;
        if(isAddBomb) {
          this.items.add(new ItemObject(this.scene, -25, Phaser.Math.Between(25, gameHeight - 100 - 25), Phaser.Math.Between(1000, 700), ItemObject.keyNameBomb));
        }
        ...
        this.items.children.iterate(child => {
          ...
          if(item.keyName === ItemObject.keyNameEnemy && item.x > gameWidth + 25) itemsToRemove.push(item);  // 敵が右端に消えた場合
          if(item.keyName !== ItemObject.keyNameEnemy && item.x <            -25) itemsToRemove.push(item);  // アイテムが左端に消えた場合
          return true;
        });
