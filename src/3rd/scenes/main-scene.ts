...
export default class MainScene extends Phaser.Scene {
  ...
  /** プリロード */
  public preload(): void {
    this.load.image(MainScene.keyNameBackground, `../2nd/${MainScene.keyNameBackground}.png`);
    this.load.image(MainScene.keyNameStatusBar , `../2nd/${MainScene.keyNameStatusBar}.png`);
    this.load.image(PlayerObject.keyName       , `../2nd/${PlayerObject.keyName}.png`);
    this.load.image(ItemObject.keyNameSora     , `../2nd/${ItemObject.keyNameSora}.png`);
    this.load.image(ItemObject.keyNameEri      , `../2nd/${ItemObject.keyNameEri}.png`);
    this.load.image(ItemObject.keyNameUnknown  , `../2nd/${ItemObject.keyNameUnknown}.png`);
    this.load.image(ItemObject.keyNameEnemy    , `../2nd/${ItemObject.keyNameEnemy}.png`);
    this.load.image(ItemObject.keyNameBomb     , `../2nd/${ItemObject.keyNameBomb}.png`);
    this.load.audio(`sound-${MainScene.keyNameGameStart}`, `../2nd/sound-${MainScene.keyNameGameStart}.mp3`);
    this.load.audio(`sound-${MainScene.keyNameGameOver}` , `../2nd/sound-${MainScene.keyNameGameOver}.mp3`);
    this.load.audio(`sound-${ItemObject.keyNameSora}`    , `../2nd/sound-${ItemObject.keyNameSora}.mp3`);
    this.load.audio(`sound-${ItemObject.keyNameEri}`     , `../2nd/sound-${ItemObject.keyNameEri}.mp3`);
    this.load.audio(`sound-${ItemObject.keyNameUnknown}` , `../2nd/sound-${ItemObject.keyNameUnknown}.mp3`);
    this.load.audio(`sound-${ItemObject.keyNameEnemy}`   , `../2nd/sound-${ItemObject.keyNameEnemy}.mp3`);
    this.load.audio(`sound-${ItemObject.keyNameBomb}`    , `../2nd/sound-${ItemObject.keyNameBomb}.mp3`);
  }
  
  /** 初期化処理 */
  public create(): void {
    // 背景 (横スクロールさせる) を配置する
    this.background = this.add.tileSprite(0, 0, gameWidth, gameHeight, MainScene.keyNameBackground).setOrigin(0, 0);
    // ステータスバーを配置する (`refreshBody()` で `setOrigin()` による当たり判定のズレを修正する)
    this.physics.add.staticImage(0, gameHeight - 50, MainScene.keyNameStatusBar).setOrigin(0, 0).refreshBody();
    // 初期状態のテキストを表示する (`setOrigin()` で中央揃えになるようにする)
    this.message = this.add.text(gameWidth / 2, gameHeight / 2, '2本指タップでスタート', { color: '#f09', fontSize: 30, fontFamily: 'sans-serif', backgroundColor: '#fff', align: 'center' }).setOrigin(0.5, 0);
    ...
    this.player = new PlayerObject(this, 50, gameHeight / 2);  // プレイヤーを用意する
  ...
  public update(): void {
    if(this.state === 'GAME_OVER') {
      if(this.input.pointer1.isDown && this.input.pointer2.isDown) {  // 2本指タップで開始する
      ...
      if(this.hpObject.hp <= 0) {  // HP が 0 になったらプレイヤー落下開始
        ...
        if(this.player.y >= (gameHeight - 50 - 25)) {
  ...
  private onCollectItem(player: PlayerObject, item: ItemObject): void {
    ...
    if(item.keyName === ItemObject.keyNameBomb) player.setY(gameHeight - 50 - 25);  // 爆弾を取った時に地面にぶつける = HP を減少させたことと合わせて即死にする
  }
}
